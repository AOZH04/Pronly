(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".productmax__slider")) var productMaxSlider = new core(".productmax__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (document.querySelector(".productmin__slider")) var productMinSlider = new core(".productmin__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            watchOverflow: true,
            navigation: {
                prevEl: ".productmin__prev",
                nextEl: ".productmin__next"
            },
            keyboard: {
                enabled: true,
                onlyInViewport: true,
        ...
        productMaxSlider.on("slideChange", (function() {
            var slideIndex = productMaxSlider.realIndex;
            productMinSlider.slideTo(slideIndex);
        }));
        productMinSlider.on("slideChange", (function() {
            var slideIndex = productMinSlider.realIndex;
            productMaxSlider.slideTo(slideIndex);
        }));
    ...
    let productRadios = document.querySelectorAll('.product_radios input[type="radio"]');
    if (productRadios) productRadios.forEach((radio => {
    ...
    const productBlocks = document.querySelectorAll(".product_pseudoradios");
    productBlocks.forEach((block => {
        const volumeButtons = block.querySelectorAll(".volume");
        volumeButtons.forEach((button => {
            button.addEventListener("click", (function(event) {
                event.preventDefault();
                volumeButtons.forEach((btn => {
                    btn.classList.remove("volumeactive");
                }));
                this.classList.add("volumeactive");
            }));
        }));
        if (volumeButtons.length > 0) volumeButtons[0].classList.add("volumeactive");
    }));
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        dropdowns.forEach((dropdown => {
            ...
            dropdown.addEventListener("click", (function() {
                ...
                dropdownContent.parentNode.classList.toggle("show");
            }));
            ...
            optionButtons.forEach((button => {
                button.addEventListener("click", (function(event) {
                    ...
                    dropdown.querySelector(".dropbtn").textContent = this.textContent;
                    if (dropdownContent.classList.contains("show")) {
                        dropdownContent.classList.remove("show");
                        dropdownContent.parentNode.classList.remove("show");
                    }
        ...
        window.addEventListener("click", (function(event) {
            dropdowns.forEach((dropdown => {
                const dropdownContent = dropdown.querySelector(".dropdown-content");
                if (!dropdown.contains(event.target)) dropdownContent.classList.remove("show");
            }));
        }));
    }));
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        var mainElement = document.querySelector("main.main_page");
        if (mainElement) {
    ...
    document.addEventListener("watcherCallback", (function(e) {
        ...
        if (entry.isIntersecting) {
            ...
            if (targetElement.classList.contains("animdown")) !targetElement.classList.contains("fadeInDown") ? targetElement.classList.add("fadeInDown") : null;
        } else {
            ...
            targetElement.classList.contains("fadeInDown") ? targetElement.classList.remove("fadeInDown") : null;
        }
    }));
    const elementsWithIndex1 = document.querySelectorAll(".photos_grid > :nth-child(10n+1)");
    if (elementsWithIndex1) for (let index = 0; index < elementsWithIndex1.length; index++) {
        const element = elementsWithIndex1[index];
        element.style.gridColumn = `span 2`;
        element.style.gridRow = `span 2`;
        element.style.gridColumnStart = `1`;
        element.style.gridColumnEnd = `3`;
        element.classList.add("animleft");
    }
    const elementsWithIndex8 = document.querySelectorAll(".photos_grid > :nth-child(10n+8)");
    if (elementsWithIndex8) for (let index = 0; index < elementsWithIndex8.length; index++) {
        const element = elementsWithIndex8[index];
        element.style.gridColumn = `span 2`;
        element.style.gridRow = `span 2`;
        element.style.gridColumnStart = `3`;
        element.style.gridColumnEnd = `5`;
        element.classList.add("animright");
    }
    let gridItems = document.querySelectorAll(".photos_grid_item");
    if (gridItems) if (gridItems.length >= 3) {
        let parentGridItem = gridItems[1].parentNode;
        parentGridItem.style.gridTemplateColumns = `repeat(auto-fill, minmax(auto, 1fr))`;
        parentGridItem.style.gridAutoRows = `${gridItems[1].offsetWidth}` + "px";
    }
    window["FLS"] = false;
    menuInit();
    functions_menuClose();
